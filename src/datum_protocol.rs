...
use crate::handlers::{MessageHandler, DefaultMessageHandler, parse_client_config, parse_job_validation, parse_share_response, parse_block_notify};
...
impl DatumProtocolClient {
    ...
    async fn perform_handshake(&self) -> Result<(), DatumError> {
        ...
        let local_keys = local_keys_guard.as_ref()
        ...
        let encrypted_hello = if let Some(pool_pk_bytes) = self.pool_public_key {
            ...
            sodiumoxide::init().map_err(|e| DatumError::EncryptionError(format!("sodiumoxide init failed: {:?}", e)))?;
            
            use sodiumoxide::crypto::box_;
            
            // Convert pool's X25519 public key to sodiumoxide format
            let pool_pk = box_::PublicKey::from_slice(&pool_pk_bytes)
                .ok_or_else(|| DatumError::EncryptionError("Invalid pool public key".to_string()))?;
        ...
        *self.header_xor_key.lock().await = header_key;
        ...
        stream.write_all(&header_xor).await
            .map_err(|e| DatumError::IoError(e))?;
        stream.write_all(&encrypted_hello).await
            .map_err(|e| DatumError::IoError(e))?;
        stream.flush().await
            .map_err(|e| DatumError::IoError(e))?;
        ...
        if command != DatumCommand::Handshake {
            return Err(DatumError::ProtocolError("Expected handshake response".to_string()));
        }
        ...
        if response_data.len() < 32 * 6 {
            return Err(DatumError::ProtocolError("Handshake response too short".to_string()));
        }
        ...
        if &response_data[offset..offset+32] != &local_ed25519_pk {
            return Err(DatumError::ProtocolError("Echoed Ed25519 key mismatch".to_string()));
        }
        offset += 32;
        
        let local_x25519_pk = local_keys.x25519_public_bytes();
        if &response_data[offset..offset+32] != &local_x25519_pk {
            return Err(DatumError::ProtocolError("Echoed X25519 key mismatch".to_string()));
        }
        offset += 32;
        
        let session_ed25519_pk = session_keys.ed25519_public().to_bytes();
        if &response_data[offset..offset+32] != &session_ed25519_pk {
            return Err(DatumError::ProtocolError("Echoed session Ed25519 key mismatch".to_string()));
        }
        offset += 32;
        
        let session_x25519_pk = session_keys.x25519_public_bytes();
        if &response_data[offset..offset+32] != &session_x25519_pk {
            return Err(DatumError::ProtocolError("Echoed session X25519 key mismatch".to_string()));
        }
        offset += 32;
        
        // Extract pool's session keys
        let pool_session_ed25519_pk_bytes: [u8; 32] = response_data[offset..offset+32].try_into()
        ...
        *self.pool_ed25519_public.write().await = Some(pool_session_ed25519_pk);
        ...
        let pool_session_x25519_pk_bytes: [u8; 32] = response_data[offset..offset+32].try_into()
        ...
        let motd_end = response_data[offset..].iter()
            .position(|&b| b == 0)
        ...
        let shared_secret = session_keys.x25519_secret.diffie_hellman(&pool_session_x25519_pk);
    ...
    pub async fn send_message(&self, command: DatumCommand, data: &[u8]) -> Result<(), DatumError> {
        if !self.connected.load(std::sync::atomic::Ordering::Acquire) {
            return Err(DatumError::PoolConnectionError("Not connected to pool".to_string()));
        }
    ...
    async fn receive_message_internal(&self) -> Result<(DatumCommand, Vec<u8>), DatumError> {
        ...
        if let Some(sig_bytes) = &signature_bytes {
            ...
            if let Some(pool_pk) = pool_pk_guard.as_ref() {
                ...
                let signature = Signature::from_bytes(sig_bytes.as_slice().try_into()
                    .map_err(|_| DatumError::ProtocolError("Invalid signature format".to_string()))?);
                
                if pool_pk.verify(&message, &signature).is_err() {
                    return Err(DatumError::ProtocolError("Signature verification failed".to_string()));
                }
        ...
        *recv_nonce_guard += 1;
        ...
        let command = DatumCommand::from_u8(header.proto_cmd)
            .ok_or_else(|| DatumError::ProtocolError(format!("Unknown command: {}", header.proto_cmd)))?;
    ...
    async fn handle_protocol_message(&self, command: DatumCommand, data: &[u8]) -> Result<(), DatumError> {
    ...
    pub fn deserialize_header_static(bytes: &[u8]) -> Result<DatumMessageHeader, DatumError> {
        ...
        let cmd_len = (bytes[0] as u32) |
                      ((bytes[1] as u32) << 8) |
                      ((bytes[2] as u32 & 0x3F) << 16);
    ...
    async fn apply_header_xor(&self, header_bytes: &[u8], xor_key: u32) -> Result<Vec<u8>, DatumError> {
        if header_bytes.len() < 4 {
            return Err(DatumError::ProtocolError("Header too short for XOR".to_string()));
        }
        ...
        let header_as_u32 = u32::from_le_bytes([
            result[0],
            result[1],
            result[2],
            result[3],
    ...
    pub async fn fetch_coinbaser(&self, value: u64) -> Result<Vec<u8>, DatumError> {
        ...
        self.send_message(DatumCommand::FetchCoinbaser, &request_data).await?;
        
        // Receive response
        let (command, data) = self.receive_message().await?;
        if command != DatumCommand::FetchCoinbaser {
            return Err(DatumError::ProtocolError("Unexpected response command".to_string()));
        }
    ...
    pub async fn submit_pow(&self, pow_data: Vec<u8>) -> Result<bool, DatumError> {
        ...
        self.send_message(DatumCommand::SubmitPow, &pow_data).await?;
        ...
        let accepted = match data[0] {
            ...
            0x66 => false, // Rejected
            _ => return Err(DatumError::ProtocolError(format!("Unknown response code: {}", data[0]))),
        };
