...
fn test_command_enum() {
    ...
    assert_eq!(
        DatumCommand::from_u8(0x11),
        Some(DatumCommand::FetchCoinbaser)
    );
    assert_eq!(
        DatumCommand::from_u8(0x99),
        Some(DatumCommand::ClientConfig)
    );
    assert_eq!(
        DatumCommand::from_u8(0x50),
        Some(DatumCommand::JobValidation)
    );
    assert_eq!(
        DatumCommand::from_u8(0x8F),
        Some(DatumCommand::ShareResponse)
    );
